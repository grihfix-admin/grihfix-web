...
const serviceOptions = [
  ...
  { label: "Bathroom Deep Cleaning", value: "bathroom" },
  { label: "Kitchen Deep Cleaning", value: "kitchen" },
  { label: "Sofa Cleaning", value: "sofa-cleaning" },
  { label: "Mattress Cleaning", value: "mattress-cleaning" },
  { label: "Chair Cleaning", value: "chair-cleaning" },
  { label: "Tiles & Floor Cleaning", value: "tiles-cleaning" },
  { label: "Chimney Cleaning", value: "chimney-cleaning" },
  { label: "Terrace / Roof Cleaning", value: "terrace" },
...
function ContactFormInner() {
  ...
  const selectedServiceOption = useMemo(
    () => serviceOptions.find((option) => option.value === formData.service) ?? serviceOptions[0],
    [formData.service]
  );
  const selectedSlot = useMemo(() => timeSlots.find((slot) => slot.value === formData.timeSlot), [formData.timeSlot]);
  const scheduleText = useMemo(() => {
    if (!formData.preferredDate) return "Team will call to confirm slot";
    const date = new Date(formData.preferredDate);
    const formatted = date.toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "short",
    });
    return `${formatted}${selectedSlot ? `, ${selectedSlot.label}` : ""}`;
  }, [formData.preferredDate, selectedSlot]);
  const isCleaningService = CLEANING_SERVICE_IDS.has(formData.service);
  const isCarWash = formData.service === carWashServiceId;
  const isTankService = TANK_SERVICE_IDS.has(formData.service);
  ...
  const orderSummary = useMemo(() => {
    const priceLabel = liveEstimate ? `₹${liveEstimate.finalPrice.toLocaleString()}` : "Will confirm post inspection";
    return {
      service: selectedServiceOption.label,
      schedule: scheduleText,
      priceLabel,
      location: formData.city || "Darbhanga",
      message: formData.message || "Team can help with the details on call.",
    };
  }, [selectedServiceOption, scheduleText, liveEstimate, formData.city, formData.message]);

  const whatsappMessage = encodeURIComponent(
    `Hi GrihFix, I want to confirm ${orderSummary.service} on ${orderSummary.schedule}. Estimated price: ${orderSummary.priceLabel}. Name: ${
      formData.name || "GrihFix customer"
    }. Phone: ${formData.phone || "NA"}.`
  );
  const whatsappHref = `https://wa.me/919709870726?text=${whatsappMessage}`;
  const successWhatsappHref = successDetails
    ? `https://wa.me/919709870726?text=${encodeURIComponent(
        `Hi GrihFix, confirming ${successDetails.service} scheduled ${successDetails.scheduleText}. Reference: ${
          successDetails.referenceId || "N/A"
        }.`
      )}`
    : undefined;
  const disableSubmit = submitState === "loading";
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isTankService && formData.tankSize.trim()) {
      detailsLines.push(`Tank size: ${formData.tankSize.trim()}`);
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const slot = timeSlots.find((item) => item.value === formData.timeSlot);
        const scheduleText = formData.preferredDate
        ...
        setSuccessDetails({
          ...
          scheduleText,
          priceLabel: liveEstimate ? `₹${liveEstimate.finalPrice.toLocaleString()}` : "To be confirmed post inspection",
          referenceId: data.booking?.trackingCode ?? data.booking?._id ?? "",
        });
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] xl:gap-12">
          <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
            {submitState === "success" && successDetails && (
              <div className="rounded-3xl border border-emerald-200 bg-emerald-50/80 p-5 text-emerald-900 shadow-inner">
                <p className="text-sm font-semibold uppercase tracking-[0.3em]">Booking received</p>
                <h3 className="mt-2 text-xl font-semibold text-emerald-900">{successDetails.service}</h3>
                <p className="text-sm text-emerald-800">Scheduled: {successDetails.scheduleText}</p>
                <p className="mt-2 text-sm text-emerald-800">Estimated price: {successDetails.priceLabel}</p>
                {successDetails.referenceId && (
                  <p className="mt-1 text-xs text-emerald-700">Reference: {successDetails.referenceId}</p>
                )}
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  {successWhatsappHref && (
                    <Button href={successWhatsappHref} target="_blank" rel="noreferrer" className="w-full justify-center sm:w-auto">
                      Confirm on WhatsApp
                    </Button>
                  )}
                  {successDetails.referenceId && (
                    <Button
                      href={`/track/${successDetails.referenceId}`}
                      variant="secondary"
                      className="w-full justify-center text-slate-900 sm:w-auto"
                    >
                      Track booking
                    </Button>
                  )}
          ...
          <div>
            ...
            {isTankService && (
              <div>
                <label className="text-sm font-medium text-slate-600" htmlFor="tankSize">
                  Tank size / capacity *
                </label>
                <input
                  id="tankSize"
                  ...
                  required
                  className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                  placeholder="e.g. 1000 L overhead + 700 L underground"
                />
                <p className="mt-1 text-xs text-slate-500">Helps us send the right suction unit and crew.</p>
              </div>
            )}

            <div className="rounded-3xl border border-slate-100 bg-slate-50 px-5 py-5 text-sm text-slate-700">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-500">Order summary</p>
                  <p className="text-lg font-semibold text-slate-900">{orderSummary.service}</p>
                  <p className="text-xs text-slate-500">City: {orderSummary.location}</p>
                </div>
                <span className="text-3xl font-bold text-slate-900">{orderSummary.priceLabel}</span>
              </div>
              <div className="mt-4 space-y-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-500">Schedule</span>
                  <span className="font-medium text-slate-900">{orderSummary.schedule}</span>
                </div>
                {isCleaningService && formData.areaRange && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">Area slab</span>
                    <span className="font-medium text-slate-900">{formData.areaRange} sq ft</span>
                  </div>
                )}
                {isTankService && formData.tankSize && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">Tank size</span>
                    <span className="font-medium text-slate-900">{formData.tankSize}</span>
                  </div>
                )}
              </div>
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                <Button
                  href={whatsappHref}
                  variant="secondary"
                  className="w-full justify-center text-slate-900"
                  target="_blank"
                  rel="noreferrer"
                >
                  Confirm on WhatsApp
                </Button>
                <Button href="tel:+919709870726" variant="ghost" className="w-full justify-center text-slate-700">
                  Call support
                </Button>
              </div>
            </div>
          <div>
            ...
            <Button type="submit" size="lg" className="w-full justify-center" disabled={disableSubmit}>
              Submit request
            </Button>
        </form>
